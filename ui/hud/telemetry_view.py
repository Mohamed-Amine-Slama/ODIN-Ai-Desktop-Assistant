...
class TelemetryPresenter:
    ...
    def render_frame(self, frame: TelemetryFrame) -> None:
        ...
        window.disk_io.set_value(
            f"{frame.disk_io.read_mbs:.1f} / {frame.disk_io.write_mbs:.1f} MB/S"
        )
    ...
    def render_weather(self, sample: WeatherSample | None) -> None:
        """Temperature, condition and the multi-day outlook. The humidity,
        wind and sunrise rows were dropped with the rest of the panel trim —
        three lines nobody was reading, refreshed every ten minutes."""
        window = self.window
        if sample is None:
